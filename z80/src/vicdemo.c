 ...
 * C128 CP/M VIC Demo!
 *
 * Modified to compile with idun-zcc and run on idun-cart
 */
...
/*
*  Compiler strings are ASCII format
*/
char *asc2pet(char[] a)
{
   int i = 0;
...
void drawEllipse(uchar *bmp, int xc, int yc, int a, int b) {
    ...
    long bb2 = bb << 1; /* 2(b^2) */
    {
        ...
        {
            setPix(bmp, (ushort)(xc + x), (char)(yc + y));
            setPix(bmp, (ushort)(xc + x), (char)(yc - y));
            setPix(bmp, (ushort)(xc - x), (char)(yc + y));
            setPix(bmp, (ushort)(xc - x), (char)(yc - y));
            x += 1;
...
void lines(uchar *bmp, uchar *scr, uchar *chr) {
    ...
    clearBmp(bmp);
    clearScr(scr, 16);
    bannerBmp(bmp, scr, chr, asc2pet(" lines "));
    for (i = 0; i < 16; i++) {
...
void rects(uchar *bmp, uchar *scr, uchar *chr) {
    ...
    clearBmp(bmp);
    clearScr(scr, 16);
    bannerBmp(bmp, scr, chr, asc2pet(" rectangles "));
    while (readKey(0) != 253) {
        x1 = (rand() / 130);
        y1 = (rand() / 210) + 10;
        x2 = x1 + (rand() / 330) + 6;
...
void ellipses(uchar *bmp, uchar *scr, uchar *chr) {
    ...
    clearBmp(bmp);
    clearScr(scr, 16);
    bannerBmp(bmp, scr, chr, asc2pet(" ellipses "));
    for (i = 1; i < 9; i++) {
...
void showColors(uchar *bmp, uchar *scr, uchar *chr) {
    ...
    clearBmp(bmp);
    clearScr(scr, 16);
    bannerBmp(bmp, scr, chr, asc2pet(" color combos for chars in bitmap "));
    for (r = 0; r < 15; r++) {
        for (c = 0; c < 15; c++) {
...
void textBounce(uchar *scr) {
    ...
    srand(inp(0xd012));
    while (readKey(0) != 253) {
        printScr(scr, x, y, color, asc2pet("fast screen output for cp/m"));
        x += xDir;
        y += yDir;
        if (x > 12) {
            xDir = -1;
...
/*
 * Scroll down and render top line on non-visible screen.
 */
